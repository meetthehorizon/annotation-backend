...
def register_user():
    ...
    try:
        ...
        db.session.add(user)
        db.session.commit()
        return jsonify({"msg": "Registration request submitted. Waiting for admin approval."}), 201
...
def approve_user(user_id):
    ...
    try:
        ...
        user.role = data.get('role', 'annotator')  # Default to annotator if not specified
        db.session.commit()
...
def create_segment(sentence_id):
    ...
    db.session.commit()
    return jsonify({
        "msg": f"{len(created)} segments added successfully",
        "segments": created
    })
...
def get_segment_usrs(segment_id):
    ...
    return jsonify([{
        ...
        "sentence_type_info": {
            'sentence_type': usr.sentence_type,
            'scope': usr.sentence_type_info[0].scope if usr.sentence_type_info else 'neutral'
        }
    } for usr in usrs])
...
def visualize_usr(segment_id):
    ...
    try:
            ...
            for line in usr_data.split('\n'):
                ...
                if not line or line.startswith(('#', '<', '%', '//')):
                    if line.startswith('#'):  # Hindi words line
                        hindi_words = {i+1: word for i, word in enumerate(line[1:].split('\t'))}
                    continue
        ...
        response.headers.set('Content-Disposition', 'inline', filename=f'usr_{segment_id}.png')
        return response
...
def parse_custom_usr_format(raw_data):
    ...
    for line in lines:
        ...
        try:
            index = int(columns[1].strip()) if columns[1].strip() and columns[1].strip() != '-' else 0
        except ValueError:
            index = 0
        ...
        lexical_item = {
            ...
            'index': index,
            'semantic_category': columns[2].strip() if columns[2].strip() != '-' else None,
            'morpho_semantic': columns[3].strip() if columns[3].strip() != '-' else None,
            'speakers_view': columns[6].strip() if columns[6].strip() != '-' else None,
            'scope': columns[7].strip() if columns[7].strip() != '-' else None  # Scope from column 7
        }
        ...
        if columns[8].strip() != '-':
            for const_rel in columns[8].strip().split():
                if ':' in const_rel:
                    ...
                    try:
                        cxn_index = int(cxn_index) if cxn_index and cxn_index != '-' else None
...
def create_usr(segment_id):
    ...
    for dep_item in data.get('dependency_info', []):
        dep = DependencyInfo(
            ...
            head_index=str(dep_item['head_index']) if dep_item['head_index'] is not None else None,
    ...
    for disc_item in data.get('discourse_coref_info', []):
        disc = DiscourseCorefInfo(
            ...
            head_index=str(disc_item['head_index']) if disc_item['head_index'] is not None else None,
    ...
    for const_item in data.get('construction_info', []):
        const = ConstructionInfo(
            ...
            cxn_index=str(const_item['cxn_index']) if const_item['cxn_index'] is not None else None,
...
def assign_usr():
    ...
    assignment = Assignment(
        ...
        assign_lexical=data.get('can_edit_lexical', False) or data.get('assign_lexical', False),
        assign_dependency=data.get('can_edit_dependency', False) or data.get('assign_dependency', False),
        assign_discourse=data.get('can_edit_discourse', False) or data.get('assign_discourse', False),
        assign_construction=data.get('can_edit_construction', False) or data.get('assign_construction', False)
    )
...
def get_all_assignments():
    ...
    assignments = db.session.query(
        ...
        User, Assignment.annotator_id == User.id
    ).join(
        Segment, Assignment.segment_id == Segment.id
    ).join(
        Sentence, Segment.sentence_id == Sentence.id
    ).join(
        Chapter, Sentence.chapter_id == Chapter.id
    ).join(
        Project, Chapter.project_id == Project.id
    ).all()
    ...
    for a in assignments:
        ...
        assignments_by_user[user_key]["assignments"].append({
            ...
            "created_at": a.Assignment.created_at.isoformat() if a.Assignment.created_at else None,
            "updated_at": a.Assignment.updated_at.isoformat() if a.Assignment.updated_at else None
        })
