...
def annotator_dashboard():
    ...
    for assignment in assignments:
        ...
        projects_dict[project.id]["chapters"][chapter.id]["assignments"].append(
            assignment_data
        )
...
def visualize_usr(usr_id):
    ...
    assignment = Assignment.query.filter_by(
        usr_id=usr_id, annotator_id=annotator.id
    ).first()
    ...
    for li in sorted(usr.lexical_info, key=lambda x: x.index):
        ...
        line_parts = [
            ...
            (
                ...
                if usr.sentence_type_info and usr.sentence_type_info[0].scope
                else "-"
            ),
            # Construction info
            " ".join(
    ...
    try:
            ...
            for line in usr_data.split("\n"):
                ...
                if not line or line.startswith(("#", "<", "%", "//")):
                    if line.startswith("#"):  # Hindi words line
                        ...
                        hindi_words = {
                            i + 1: word
                            for i, word in enumerate(line[1:].strip().split())
                        }
                    continue
        ...
        response.headers.set(
            "Content-Disposition", "inline", filename=f"usr_{usr_id}.png"
        )
        return response
    ...
    return jsonify(
        {
            ...
            "usr": {
                ...
                "dependency_info": (
                    dependency_info if assignment.assign_dependency else []
                ),
                "discourse_coref_info": (
                    discourse_coref_info if assignment.assign_discourse else []
                ),
                "construction_info": (
                    construction_info if assignment.assign_construction else []
                ),
...
def update_usr(usr_id):
    ...
    try:
        ...
        payload_lex = {
            str(item.get("id")): item for item in data.get("lexical_info", [])
        }
        ...
        payload_dep = {
            str(item.get("id")): item for item in data.get("dependency_info", [])
        }
        ...
        payload_disc = {
            str(item.get("id")): item for item in data.get("discourse_coref_info", [])
        }
        ...
        payload_const = {
            str(item.get("id")): item for item in data.get("construction_info", [])
        }
